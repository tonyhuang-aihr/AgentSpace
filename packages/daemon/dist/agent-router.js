...
async function runCommand(args) {
  ...
  const outputFormat = parsed.flags["json-events"] ? "json-events" : "text";
  ...
  const request = {
    ...
    timeoutMs: timeoutRaw ? Number(timeoutRaw) : void 0,
    outputFormat
  };
...
function parseAgentRouterRunArgs(args) {
  const positionals = [];
  const flags = {};
  const booleanFlags = /* @__PURE__ */ new Set(["json-events"]);
